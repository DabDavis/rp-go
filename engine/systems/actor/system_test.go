...
func TestRegistryEntitiesReturnsSortedCopy(t *testing.T) {
	w := ecs.NewWorld()
	...
	actors := system.Registry().Entities()
	if len(actors) != 3 {
	...
	again := system.Registry().Entities()
	if len(again) != 3 {
		t.Fatalf("expected registry.Entities to return copy, got %d entries", len(again))
	}
	if again[0] == nil {
		t.Fatalf("expected registry.Entities copy to remain unchanged")
	}
}
